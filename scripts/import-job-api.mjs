...
export async function openAIEdit({ key, baseUrl, model, prompt, images, size, background, quality, count = 1, outputFormat = "png", moderation = "auto", timeoutMs = 300000, compressInput = false }) {
  const parsedTimeout = Number(timeoutMs);
  const requestTimeoutMs = Number.isFinite(parsedTimeout) && parsedTimeout > 0 ? parsedTimeout : 300000;
  // OpenLux follows the documented gateway schema for multi-image edits:
  // repeat the `image` field (not OpenAI's `image[]` spelling), and request
  // base64 output so the local app can persist the result without a second
  // unauthenticated CDN download.
  const openLux = /(^|:\/\/)(?:api\.)?openlux\.ai(?:\/|$)/i.test(String(baseUrl || ''));
  // Always send the exact model name entered by the user.  The provider may
  // reject a model/request combination, but silently switching to another
  // model can produce an unexpected result (and an unexpected charge).
  // OpenLux's documented gpt-image-2 edit schema only accepts model, prompt,
  // image, n and response_format. Its 2.5 edit schema adds size/quality/
  // format/moderation/background.  Decide which optional fields are safe to
  // send based on the entered model, without changing that model value.
  const effectiveModel = String(model || '').trim();
  const openLuxExtended = !openLux || /^gpt-image-2\.5(?:-|$)/i.test(effectiveModel);
  ...
  if (openLuxExtended) {
    form.set("size", size);
    // OpenLux edit gateways may reject `format` even for some 2.5 models;
    // the local persistence layer converts the returned image afterward.
    if (!openLux) form.set("output_format", outputFormat);
    form.set("quality", quality || "high");
    form.set("moderation", moderation);
    if (background) form.set("background", background);
  }
  // Original JPEG/PNG/WebP bytes include their original orientation and colour
  // metadata. Only an explicit retry may encode a smaller sending copy.
  const inputs = await prepareImageInputs(images, { compressInput });
  for (const image of inputs) {
    form.append(openLux ? "image" : "image[]", new Blob([image.data], { type: image.mime }), image.name);
  }
  ...
  try {
    ...
    try { result = await response.json(); }
      ...
      if (response.status === 413) throw new Error("图像接口拒绝了过大的上传请求（HTTP 413）。可选择压缩参考图后重试。", { cause: error });
      throw new Error("OpenAI image response was not valid JSON", { cause: error });
    }
  } catch (error) {
    const failure = error instanceof Error ? error : new Error(String(error));
    failure.isLocalTimeout = signal.aborted;
    failure.timeoutMs = requestTimeoutMs;
    failure.status = response?.status;
    if (response?.status === 413) failure.failureKind = "input_too_large";
    failure.requestId = response?.headers?.get("x-request-id") || response?.headers?.get("request-id") || response?.headers?.get("openai-request-id") || null;
    // Node's fetch often surfaces network failures only as the generic
    // "fetch failed" message. Preserve the underlying socket/DNS code so the
    // local job record can explain that this was a connection problem rather
    // than a prompt or image validation error.
    failure.networkCode = error?.cause?.code || error?.code || null;
    failure.networkMessage = error?.cause?.message || null;
    throw failure;
  }
  const requestId = response.headers.get("x-request-id") || response.headers.get("request-id") || response.headers.get("openai-request-id") || result?.request_id || null;
  if (!response.ok) {
    const error = new Error(result?.error?.message || `OpenAI image request failed (${response.status})`);
    error.status = response.status;
    if (response.status === 413) error.failureKind = "input_too_large";
    error.requestId = requestId || error.message.match(/request\s*id\s*[:：]\s*([A-Za-z0-9_-]+)/i)?.[1] || null;
    error.providerCode = result?.error?.code || result?.error?.type || null;
    throw error;
  }
  const outputs = [];
  for (const item of (result.data || [])) {
    ...
    if (item?.b64_json) {
      ...
      // Some OpenAI-compatible gateways still return a short-lived URL even
      // when b64_json was requested. Download it immediately while the
      // provider URL is valid; the local app never stores that remote URL.
      const imageResponse = await fetch(item.url, { signal: AbortSignal.timeout(requestTimeoutMs) });
      if (!imageResponse.ok) throw new Error(`图像服务返回的图片地址不可访问（HTTP ${imageResponse.status}）`);
      bytes = Buffer.from(await imageResponse.arrayBuffer());
    }
    if (!bytes?.length) continue;
    Object.defineProperties(bytes, {
      requestId: { value: requestId, enumerable: false },
      httpStatus: { value: response.status, enumerable: false },
      modelUsed: { value: effectiveModel, enumerable: false },
    });
    outputs.push(bytes);
  }
  if (!outputs.length) throw new Error("OpenAI response did not contain image data");
