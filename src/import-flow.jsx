  ...
  return (
    <div className="import-editor">
      ...
      <div className="import-fields">
        <p className="import-editor__stage">{isCrop ? "衣物裁剪" : isGarment ? "服装图片" : "搭配效果图"}</p>
        ...
        <div className="import-actions">
          <button className="import-button" disabled={busy} onClick={() => onAction("reject")}><Trash size={14} /> 删除</button>
          ...
          <button className="import-button import-button--primary" disabled={busy || (isGarment && (!draft.name.trim() || !primaryValid || !secondaryValid))} onClick={() => onAction("approve")}><Check size={14} weight="bold" /> {isCrop ? "使用裁剪图" : "确认"}</button>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="import-cleanup-editor">
      ...
      <div className="import-actions">
        ...
        <button className="import-button import-button--primary" disabled={busy} onClick={onAccept}><Check size={14} weight="bold" /> 使用此结果</button>
      </div>
    </div>
  );
}
