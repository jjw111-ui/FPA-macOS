...
function ColorControl({ label, field, value, palette, onChange, sampling, setSampling, optional = false, onClear, onAdd }) {
  if (optional && !value) {
    return (
      <div className="color-slot empty-color-slot">
        ...
        <p>未检测到明显的第二颜色。</p>
        <button className="add-secondary-button" type="button" onClick={onAdd}>添加第二颜色</button>
      </div>
    );
  }
  ...
  return (
    <div className="color-slot">
      ...
      >
        {sampling === field ? "取消取色" : `从图片提取${label}`}
      </button>
    </div>
  );
}
...
function ItemViewer({ item, onClose, onSave, onDelete }) {
  ...
  const handleImageClick = (event) => {
    ...
    if (!color) {
      setSampleStatus("该位置是透明区域，请直接点击衣物。");
      return;
    }
    ...
    setSampleStatus(`已提取 ${color} 作为${sampling === "secondary" ? "第二" : "主"}颜色。`);
    setSampling(null);
  };
  ...
  return (
    ...
    <aside className={`viewer editing${hasModeledImage ? " has-modeled-image" : ""}${shaking ? " shake" : ""}`} role="dialog" aria-modal="true" aria-label="Selected wardrobe item">
      <button className="viewer-icon-close" type="button" onClick={requestClose} aria-label="关闭详情" ref={closeButtonRef}>
  ...
  const deleteItem = async (id) => {
    if (id.startsWith("import-")) {
      try {
        ...
        if (!response.ok && response.status !== 404) throw new Error("无法删除该衣物。");
      } catch (requestError) {
      ...
      <main className="gallery-pane">
        <header className="gallery-header">
          <div className="gallery-meta-row">
            <p className="piece-count">{items.length} 件衣物</p>
          </div>
          <nav className="category-nav" aria-label="按衣物分类筛选">
            {TYPES.map((type) => (
              <button
                key={type.id}
                type="button"
        ...
        {!error && !loading && !items.length && <p className="status empty">拖入、粘贴或添加照片，开始建立衣橱。</p>}
